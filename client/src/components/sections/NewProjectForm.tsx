    ...
    return (
        <><form className={css.form} onSubmit={(e)=>createProject(e)}>
            ...
            <input
                ...
                required
                onChange={(e) => setFormValues({ ...formValues, organization_foto_src: e.target.files && e.target.files[0] })}
            />
            ...
            />
            <p onClick={()=>setSocialMediaOpened(!isSocialMediaOpened)} className={css.socialMediaTitle}><span>{isSocialMediaOpened ? 'Close' : 'Add' }</span> social media <span>{isSocialMediaOpened ? '-' : '+' }</span></p>
            {isSocialMediaOpened && 
                <>
